...
//!
//! Ported almost verbatim from `apps/tauri/src-tauri/src/e2ee_client.rs`.
...
//!
//! Some wire-shape fields + accessor methods are carried over verbatim for
//! parity even though the streamlined spike orchestrator doesn't read them
//! all yet; `#![allow(dead_code)]` keeps the port faithful without noise.
#![allow(dead_code)]
...
struct ObjectWriteWire {
    ...
    #[serde(
        rename = "collectionVersion",
        default,
        deserialize_with = "de_opt_u64_lenient"
    )]
...
impl E2eeClient {
    ...
    // ── Endpoint 13: GET /api/sync/events (SSE, long-lived) ──
    //
    // Opens the server's Server-Sent Events stream (`ready` / `change` /
    // `ping`). Returns the raw response; the caller consumes `.bytes_stream()`.
    // Uses a DEDICATED client WITHOUT the 30s total-request timeout (which
    // would tear down the stream); a connect timeout still fails a dead host
    // fast. Liveness past connect is the watch loop's read-idle watchdog.
    pub async fn open_event_stream(&self) -> Result<reqwest::Response, E2eeHttpError> {
        let url = self.join("api/sync/events")?;
        ...
        let res = stream_http.get(url).headers(headers).send().await?;
        if !res.status().is_success() {
            let status = res.status().as_u16();
            let body = res.text().await.unwrap_or_default();
            return Err(E2eeHttpError::Http { status, body });
...
async fn parse_json_or_http<T: for<'de> Deserialize<'de>>(
    ...
    serde_json::from_slice(&bytes).map_err(|e| {
        E2eeHttpError::InvalidJson(format!(
            "{e} (body: {:?})",
...
mod tests {
    ...
    #[test]
    fn parse_iso_ms_matches_js_date_get_time() {
        // Reference: matches `new Date('2026-05-13T00:00:00.000Z').getTime()`.
    ...
    fn auth_mode_from_str() {
        assert_eq!(AuthMode::from_str("dev"), AuthMode::Dev);
        assert_eq!(AuthMode::from_str("password"), AuthMode::Password);
        assert_eq!(AuthMode::from_str("anything-else"), AuthMode::Password);
    }

    #[test]
    fn base_url_normalizes_trailing_slash() {
        let c = E2eeClient::new("http://127.0.0.1:3100").unwrap();
        assert_eq!(c.base_url(), "http://127.0.0.1:3100/");
        let c2 = E2eeClient::new("http://127.0.0.1:3100/").unwrap();
        assert_eq!(c2.base_url(), "http://127.0.0.1:3100/");
    }

    // ── HTTP-contract tests (wiremock) ──
    // Ported from apps/tauri/src-tauri/src/e2ee_client.rs. These pin the exact
    // wire shapes the server emits and the client must accept.
    ...
    async fn put_key_material_sends_expected_body() {
        let server = MockServer::start().await;
        let (_, material) = futo_notes_core::e2ee::wrap_vault_key("password").unwrap();
