...
the one `cargo-ndk` builds the Rust `.so` with breaks NDK resolution and makes AGP *silently* skip
...
1. **Refused** (exit 2, before any network call) — recipes that need Xcode, the iOS simulator or
   ...
   `deploy-ios`, `lint-swift`, every `sim-*`), recipes whose *purpose* is the shipped desktop engine
...
3. **Clean** — everything else, including `check`. `just check` is tsc, eslint, prettier,
   ...
   and carries no caveat. `prepush` is caveated rather than refused precisely because it *adds* the
...
`node_modules` is absent) — 40 seconds most runs do not need, and skipping it *silently* would be
...
that lack of isolation is fine *because* the lock makes runs serial. A blocked run reports who holds
...
because the QA tooling assumes a *local* `adb`: `just qa-claim`, `just android-drive` and
