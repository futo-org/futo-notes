...
| # | Decision | Why |
|---|---|---|
| 1 | Remove `gate-redproofs` from `check:arch-gate:portable` and `prepush`, but retain the harness, fixtures, and package script (invoked directly as `node scripts/gate-redproofs.mjs --include-cargo`; the standalone `just gate-redproofs` wrapper was later removed for having zero invocations). | A gate about the gates was re-proving unchanged conclusions on every `just check` and CI `test:` job. Its evidence is still valuable when a gate changes, so the targeted command remains without the per-commit cost. |
