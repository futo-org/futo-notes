...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B1  | **Underscore escaping rewrites notes.** Any keystroke in a note containing `snake_case_word` saves `snake\_case\_word`; a tag `#dog_problems` becomes `#dog\_problems`, stops parsing as a tag, and every chip on the note disappears. Surfaced because the desktop tag bar now round-trips through the editor, but it applies on every platform. | spec "Tags" Gap; `tests/tags.spec.ts` (`test.fixme` naming it) | A narrowed `_` escape next to `withNarrowedAtxHashEscape` in `packages/editor/src/milkdown-compat/stringifyHandlers.ts`, measured with `just milkdown-census --diff` against the ~31k-note corpus before landing (`packages/editor/AGENTS.md`). |
| B2  | **Undo can revive a superseded version.** A sync adopt or host content push is an ordinary undoable transaction, so one Ctrl-Z after a peer's edit lands restores the version it replaced and hands that to autosave. CodeMirror applied external content with `addToHistory: false` for exactly this reason.                                     | spec "Saving & rename" Gap                                     | `MilkdownEditor.svelte` `applyExternal`: dispatch the replace with `addToHistory: false`. Touches the progressive-load history baseline (CRITICAL area) — test the interaction with the save lock.                                              |
