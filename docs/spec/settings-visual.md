...
| Concern             | Desktop / Tauri (`SettingsScreen.svelte`)                                                                                                                                                                                                                        | Native Android (`apps/android/.../ui/SettingsScreen.kt`)                                                                                      | Native iOS (`apps/ios/Sources/Settings/SettingsView.swift`)                                                                                   |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Account header      | None                                                                                                                                                                                                                                                             | None — folded into the single "Sync" row (see settings.md)                                                                                    | None — folded into the single "Sync" row                                                                                                      |
| Sync UI             | **Inline** in the Sync card (URL, password, Connect / Sync now, links)                                                                                                                                                                                           | A single **"Sync" row that routes to a separate Sync screen** (`onOpenSync`); no inline form                                                  | A single **"Sync" row** (with SYNCED/LOCAL badge) that presents `SyncView` as a sheet; no inline form                                         |
| Storage / notes dir | **Storage** section — shows path, a danger-colored "This folder is no longer reachable…" line while the vault is missing, "Change directory" (folder picker + app restart), "Reset to default"                                                                   | **Storage** section — "Storage location" path readout + storage-**mode** switcher (Device/App storage — see app.md); no free directory picker | **Storage** section — read-only "Notes folder" path readout (selectable, monospaced); no picker (fixed sandbox)                               |
...
| Issue reporting     | Present — "Share crash reports", nested "Send crashes automatically" (shown only while the first is on), and "Report an issue" link, welded into one grouped card. All three rows share the same 16px left inset — nesting is never drawn as a horizontal indent | Present — "Share crash reports" `Switch`, nested "Send crashes automatically" (shown only while the first is on), and "Report an issue" link  | Present — "Share crash reports" `Toggle`, "Send crashes automatically" `Toggle` (disabled while the first is off), and "Report an issue" link |
...
- **Issue reporting** — opt-in **"Share crash reports"**; when on, a nested
  **"Send crashes automatically"** option. "Nested" here means _dependent_, not
  _indented_: every shell expresses the dependency by presence or enablement
  (desktop and Android show the row only while the parent is on; iOS disables
  it), and on all three the row keeps the same leading inset as its siblings.
  A **"Report an issue"** link opens
...
- **One filled button per card.** A Settings card has at most one solid,
  filled action — the thing that card exists to do. Everything beside it is a
  bordered or text-weight control, in whatever idiom the shell uses: two filled
  slabs of equal weight read as two equally likely choices, and they are not.
  _(desktop)_ The Storage card was the counterexample until 2026-09-18 —
  **Change directory** was a full-width filled black slab, which made the one
  piece of housekeeping in Settings its loudest element; it is now a bordered
  secondary control (`settings-btn-secondary`) beside the **Reset to default**
  link. The License plate states the same law for Buy/Renew (license.md
  § States and copy). → `src/features/settings/settingsFields.css`
