...
- Folder names must be unique among siblings (case-insensitive) and
  ...
  supplies the noun rather than the manifest. iOS reaches the same
  `forbidden_chars` verdict over FFI and shares one validator between its
  create and rename dialogs (`FolderNameValidation.swift`
  `folderNameProblem`), so a forbidden character is named in both rather than
  dropped on commit. A committed create toasts
  ...
  > **Gap:** _(Android)_ **Android** does not name a forbidden character.
  > Its New Folder dialog gates Create on three conditions only — empty after
  > sanitize, a case-insensitive duplicate, or a name that sanitizes away
  > entirely (`clean == "Untitled" && raw != "Untitled"`) — and shows
  ...
  > message. Measured on emulator 2026-09-04.
  > → NewFolderDialog.kt
