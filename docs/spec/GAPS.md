...
- [nav.md:40](nav.md#L40) — the shared editor's mount-time auto-focus is unconditional and is NOT gated off when `nativeShell` identifies the native embed, so the editor focuses itself on mount rather than staying unfocused until the host asks. The sibling native selection-toolbar and pointer-selection behaviors are correctly gated by `nativeShell`; applying the same gate here remains pending because its runtime effect on the pre-warmed native WebView (whether/when it pops the soft keyboard) needs separate on-device QA. → MarkdownEditor.svelte mount auto-focus *(native shells)*
