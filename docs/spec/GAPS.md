...
- [editor.md:455](editor.md#L455) — iOS still flushes with a plain exists-then-write and a hand-synced pending-draft register — no compare-and-swap, so a delete/adopt racing the flush can resurrect or clobber. iOS adopts the compare-and-swap primitive + the derived register in PKT-10 (needs a macOS host).
...
- [list.md:241](list.md#L241) — the native shells expose no folder-rename affordance yet — the folder long-press menu offers Delete only (iOS `NoteListView.swift`, Android `NoteListScreen.kt`), and the shared `NoteStore` FFI facade has no rename-folder primitive. *(native shells)*
- [list.md:249](list.md#L249) — the native shells can move a *note* into a folder ("Move to Folder…") but expose no folder-move affordance — moving a folder itself belongs in the folder long-press menu alongside Rename and Delete, and the shared `NoteStore` FFI facade has no move-folder primitive. *(native shells)*
