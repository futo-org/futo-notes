...
| Sync consumers (`server/`, `session/connect.rs`, `sync/*.rs`, `checkpoint.rs`) | Current protocol/orchestration | Internal workspace with shipped wire behavior | **Lifecycle owner is SyncSession** | Imports merge/conflict names through unrelated E2EE re-exports | Import `merge` and `conflict_names` directly; otherwise preserve |
