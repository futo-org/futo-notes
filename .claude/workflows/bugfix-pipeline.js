...
const DIAGNOSIS_SCHEMA = {
  ...
  properties: {
    ...
    reproduced: { type: 'boolean', description: 'true only if the wrong behavior was actually observed' },
    reproSteps: { type: 'string' },
    rootCause: { type: 'string', description: 'the underlying cause, not the symptom' },
    affectedFiles: { type: 'array', items: { type: 'string' } },
    testType: {
      type: 'string',
      enum: ['rust-unit', 'vitest-unit', 'playwright-e2e', 'shared-unit', 'cross-platform', 'conformance', 'other'],
    },
    testPath: { type: 'string', description: 'path to the failing test that was written' },
    testName: { type: 'string' },
    confirmedFailing: { type: 'boolean', description: 'true only if the new test was run and failed for the right reason' },
...
const VERIFY_SCHEMA = {
  ...
  properties: {
    ...
    regressions: { type: 'array', items: { type: 'string' }, description: 'tests/behaviors the fix broke' },
    addressesRootCause: { type: 'boolean' },
    gamingRisk: { type: 'boolean', description: 'true if the fix looks like it games the test green' },
...
log(`Done: ${summary.fixed}/${summary.total} fixed · ${summary.couldNotReproduce} not reproduced · ${summary.unresolved} unresolved.`)
