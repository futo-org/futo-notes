...
const LANDMINES = [
  {
    key: 'emoji-control',
    focus:
      'sanitizeTitle / validateTitle with emoji, ZWJ sequences, control chars, RTL, combining marks near the 200-UTF-16-unit length limit',
  },
  {
    key: 'code-fences',
    focus:
      'extractTags with nested ```/~~~ fences, unclosed fences to EOF, info strings, inline-code backtick runs, tags adjacent to punctuation/emoji',
  },
  {
    key: 'paths',
    focus:
      'validateFolderPath / pathDepth with ../ traversal, ./ , double-slash, depth > 10, Windows-reserved components, mixed case',
  },
  {
    key: 'tag-grammar',
    focus:
      'normalizeTagName / isValidTagName / tagRegexMatches with leading #s, whitespace runs, uppercase, hyphen/underscore edges, 50/51-char boundary',
  },
];
const FUZZ_SCHEMA = {
  type: 'object',
      ...
      items: {
        ...
        properties: {
          ...
          isRealBug: {
            type: 'boolean',
            description:
              'false if a documented representation difference (e.g. non-ASCII byte offset)',
          },
        },
      },
    },
...
const realBugs = fuzz
  .filter(Boolean)
  .flatMap((r) =>
    (r.divergences || []).filter((d) => d.isRealBug).map((d) => ({ ...d, category: r.category })),
  );
