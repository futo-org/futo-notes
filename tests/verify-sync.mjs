...
async function main() {
  ...
  const created = await executeJs(
    ws,
    `(async () => { const m = await import('/src/lib/notes.ts'); const r = await m.createNote(${JSON.stringify(noteTitle)}, '# Hello\\nTest body for sync verification'); return r; })()`,
  );
  ...
  log('status', {
    ...
    serverUrl: status?.preferences?.sync?.serverUrl,
    hasToken: Boolean(status?.preferences?.sync?.token),
    fileCount: Object.keys(status?.appState?.v2SyncState?.fileHashes || {}).length,
  });
