...
const filename = {
  ...
  groups: [
    group('sanitizeTitle', 'sanitizeTitle', sanitizeTitle, [
      'hello-world', 'a<b>c:d', 'a<b>c:d"e|f?g*h', '..hidden', 'file..',
      'v2.0 notes', 'Dr. Smith', '...', '.', ALPHA_300, '', '   ',
      'a\x00b\x1fc', 'a\x7fb', '  hello  ',
    ]),
    group('validateTitle', 'validateTitle', (s) => kinds(validateTitle(s)), [
      'my note', 'a<b', '.hidden', 'file.', ALPHA_201, '', '   ',
      '.<bad>.', 'v2.0 notes', 'Dr. Smith', 'a\x7fb',
    ]),
    group('isValidTitle', 'isValidTitle', isValidTitle, [
      'my note', '.hidden', 'a<b', '',
    ]),
    group('isWindowsReservedName', 'isWindowsReservedName', isWindowsReservedName, [
      'CON', 'con', 'Con', 'PRN', 'prn', 'AUX', 'NUL',
      'COM1', 'COM2', 'COM3', 'COM4', 'COM5', 'COM6', 'COM7', 'COM8', 'COM9',
      'lpt1', 'lpt2', 'lpt3', 'lpt4', 'lpt5', 'lpt6', 'lpt7', 'lpt8', 'lpt9',
      'CON.md', 'lpt5.txt', 'COM10', 'hello', 'confidential',
    ]),
    group('validateFolderName', 'validateFolderName', (s) => kinds(validateFolderName(s)), [
      'Specs', 'CON', '.hidden', 'a/b', 'a\\b', 'a:b', '',
    ]),
    group('isValidFolderName', 'isValidFolderName', isValidFolderName, [
      'Specs', 'CON', 'lpt9',
    ]),
    {
    ...
    group('validateFolderPath', 'validateFolderPath', (s) => kinds(validateFolderPath(s)), [
      'Specs', 'Specs/Folder', 'a/b/c', TOO_DEEP, 'CON/foo', 'a/PRN',
      'a//b', 'a/./b', 'a/../b',
    ]),
    group('isValidFolderPath', 'isValidFolderPath', isValidFolderPath, [
      'Specs', 'Specs/Folder', 'a/b/c', 'CON/foo', 'a/PRN', 'a//b', 'a/./b', 'a/../b',
    ]),
    group('pathDepth', 'pathDepth', pathDepth, [
      'foo', 'a/foo', 'a/b/c/foo',
    ]),
  ],
};
...
const tags = {
  ...
  groups: [
    group('tagRegexMatches', 'TAG_REGEX', tagRegexMatches, [
      '#recipes #cooking', '#123 #a1 #Z', '#meal-prep #to_do',
      '# heading\n## heading2', '#tag\n#another', 'example.com#section foo#bar',
    ]),
    group('isValidTagName', 'isValidTagName', isValidTagName, [
      'recipes', 'meal-prep', 'to_do', 'a',
      '', '123', '-start', '_start', 'React', 'dog problems', ALPHA_51,
    ]),
    group('normalizeTagName', 'normalizeTagName', normalizeTagName, [
      'Whale', 'dog problems', '#Dog   Problems',
    ]),
...
const image = {
  ...
  groups: [
    group('isImageFilename', 'isImageFilename', isImageFilename, [
      'photo.jpg', 'photo.jpeg', 'image.png', 'animation.gif', 'modern.webp',
      'vector.svg', 'bitmap.bmp', 'icon.ico', 'next-gen.avif', 'apple.heic',
      'photo.JPG', 'photo.Png', 'photo.WEBP',
      'note.md', 'file.txt', 'script.js', 'style.css', 'archive.zip',
      'noextension', '.hidden',
      '1234567890-abc.jpg', '1742345678901-xk7.png',
    ]),
    {
      op: 'imageExtensions',
      fn: 'IMAGE_EXTENSIONS',
...
void banner;
if (check && stale) {
  console.error('Conformance fixtures are stale. Run: pnpm exec tsx tests/conformance/generate.mjs');
  process.exit(1);
}
