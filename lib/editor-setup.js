    ...
    try {
      ...
      enter: (node) => {
        ...
        if (type.startsWith('ATXHeading')) {
          ...
          // Get the URL
          const imageUrl = url ? view.state.doc.sliceString(url.from, url.to) : '';
            ...
            for (let lineNum = tableStartLine; lineNum <= tableEndLine; lineNum++) {
              ...
              if (lineNum > tableStartLine) {
                decorations.push({
                  from: line.from,
                  to: line.from,
                  value: Decoration.line({ class: 'cm-md-table-hidden-line' })
                });
              }
            }
          }
      ...
      return Decoration.set(decorations.map(d => d.value.range(d.from, d.to)), true);
    } catch (e) {
