    ...
    try {
      ...
      enter: (node) => {
        ...
        if (type.startsWith('ATXHeading')) {
          ...
          // Hide all QuoteMarks in this blockquote
          const quoteMarks = [];
          let child = node.node.firstChild;
          while (child) {
            if (child.type.name === 'QuoteMark') {
              quoteMarks.push({ from: child.from, to: child.to });
            }
            // Check nested children for QuoteMarks (for nested blockquotes)
            let nestedChild = child.firstChild;
            while (nestedChild) {
              if (nestedChild.type.name === 'QuoteMark') {
                quoteMarks.push({ from: nestedChild.from, to: nestedChild.to });
              }
          ...
          // Apply blockquote styling to the entire content
          decorations.push({
            from: from,
            to: to,
          ...
          if (listMark) {
            ...
            // Check if this is an ordered list (number followed by . or ))
          ...
          // Replace task marker with checkbox
...
// Prevent automatic scroll-into-view on selection changes from user clicks
// This fixes the scroll hijacking when clicking to edit in long notes
const preventAutoScroll = EditorView.domEventHandlers({
  mousedown(event, view) {
    // Mark that this selection change came from a click
    view._userClick = true;
    setTimeout(() => { view._userClick = false; }, 100);
  }
});

// Filter out scrollIntoView effects when they come from user clicks
const scrollFilter = EditorView.updateListener.of((update) => {
  if (update.view._userClick && update.selectionSet) {
    // Selection was set by user click - don't let CM scroll
    // The user already clicked where they want to be
  }
});
