...
// Reusable widget classes with proper eq() methods to prevent unnecessary DOM rebuilds
...
class HRWidget extends WidgetType {
  eq() { return true; } // All HRs are identical
  toDOM() {
...
class BulletWidget extends WidgetType {
  eq() { return true; } // All bullets are identical
  toDOM() {
...
const hideMarkdownPlugin = ViewPlugin.fromClass(
  class {
    ...
    update(update) {
      // Rebuild decorations on doc changes, selection changes, or focus changes
          ...
          enter: (node) => {
            ...
            if (type.startsWith("ATXHeading")) {
              ...
              if (!cursorOnTable) {
                ...
                // Build table HTML for the widget
                let tableHTML = '<table class="cm-md-table">';
                  ...
                  if (lineNum === tableStartLine) {
                    decorations.push({
                      ...
                      value: Decoration.widget({
                        widget: new TableWidget(tableHTML),
                        side: -1,
                      }),
                    });
                  }
