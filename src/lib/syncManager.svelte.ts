...
export function createSyncManager(deps: SyncManagerDeps): SyncManager {
  ...
  async function handleSingleWatcherEvent(event: FileChangeEvent): Promise<void> {
    ...
    // The Rust watcher already fed the owned index; this refresh only projects
    // the new durable snapshot into reactive presentation state.
  ...
  async function handleSyncComplete(summary: SyncSummary, trigger?: SyncTrigger): Promise<void> {
    ...
    // own push back through `summary.updatedIds` forced a redundant full-vault
    // reconcile on every keystroke-triggered sync. Pure pushes leave the
    // projection cache and owned search index correct, so there is no rescan.
    ...
    if (hasPeerNoteChanges) {
      // Sync applies under watcher suppression, so ask the same store owner for
      // one corpus reconcile after a peer-driven batch.
