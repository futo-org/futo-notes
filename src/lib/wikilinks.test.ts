...
describe('shortestUniqueSuffix', () => {
  ...
  it('falls back to full ID when no shorter suffix is unique', () => {
    // Here the same path-tail recurs at multiple levels.
    const ids = ['x/A/B/grocery', 'A/B/grocery'];
    // For x/A/B/grocery: B/grocery collides with A/B/grocery. A/B/grocery collides too. Need x/A/B/grocery.
...
describe('rewriteWikilinks', () => {
  ...
  it('handles a folder rename that moves multiple notes', () => {
    // Spec § Wikilink rewrite-on-move: "Folder rename/move dispatches
    // this for every contained note." This is integration-level — here
    // we just verify rewrite handles each note correctly.
