...
// Mock the stateless updater helpers; the coordinator under test wraps these.
...
// start() gates the poll on the "automatically check for updates" pref.
const loadPreferencesMock = vi.fn(async () => ({ updates: { enabled: true } }));
vi.mock('./appState', () => ({ loadPreferences: loadPreferencesMock }));
...
/** Re-import the module so each test gets a fresh singleton. */
...
describe('check (silent / background)', () => {
  ...
  it('error → swallowed, never wedges on "checking", no banner', async () => {
    ...
    // A failed/hung background check must NOT leave the shared phase on
    // 'checking' (which would disable the Settings button).
  ...
  it('a hung background check never blocks the manual Settings button', async () => {
    const upd = await fresh();
    // Background check that never resolves (simulates a dead endpoint).
    checkMock.mockReturnValue(new Promise(() => {}));
    void upd.check({ silent: true });
    // busy drives the Settings button's disabled state; it must stay false.
  ...
  it('does not clobber an engaged error state (preserves the retry banner)', async () => {
    const upd = await fresh();
    // Engaged install that failed → error banner with retry.
    checkMock.mockResolvedValue(makeUpdate('2.0.0'));
    await upd.check();
    installMock.mockRejectedValue(new Error('verify failed'));
    await upd.install();
    expect(upd.phase).toBe('error');
    // Hourly silent poll finds the same update — must NOT wipe the error.
    checkMock.mockResolvedValue(makeUpdate('2.0.0'));
    await upd.check({ silent: true });
    expect(upd.phase).toBe('error');
  ...
  it('a NEWER version overrides a stuck error (transient failure must not wedge auto-update)', async () => {
    const upd = await fresh();
    // Engaged install of 2.0.0 fails → error.
    checkMock.mockResolvedValue(makeUpdate('2.0.0'));
    await upd.check();
    installMock.mockRejectedValue(new Error('verify failed'));
    await upd.install();
    expect(upd.phase).toBe('error');
    // A genuinely newer release ships later; the hourly poll MUST surface it
    // rather than stay wedged on the failed version forever.
  ...
  it('retracts a stale "available" when the offered release is gone (yanked)', async () => {
    ...
    await upd.check({ silent: true });
    expect(upd.phase).toBe('available');
    // Release pulled → endpoint now reports no update. The banner must not keep
    // advertising a version the server no longer serves.
    checkMock.mockResolvedValue(null);
    await upd.check({ silent: true });
    expect(upd.phase).toBe('idle');
...
describe('install', () => {
  it('forwards progress, then auto-relaunches when installUpdate returns', async () => {
    ...
    // installUpdate normally relaunches (never returns); if it resolves we
    // finish the job automatically — no manual "Restart now" click. The phase
    // rests on 'restart' since the mocked relaunch doesn't kill the process.
...
describe('start', () => {
  ...
  it('a stop()+start() while the support gate is awaiting does not leak an interval', async () => {
    ...
    // Only the live start() (p2) should have armed a timer + fired the immediate
    // check. A leaked interval from p1 would double every tick.
...
describe('disable', () => {
  ...
  it('a silent check resolving after disable() does not resurrect the banner', async () => {
    ...
    await inFlight;
    // With the poll stopped, nothing would ever retract a banner raised here.
    expect(upd.phase).toBe('idle');
    expect(upd.pending).toBeNull();
    expect(upd.bannerVisible).toBe(false);

    // Re-enabling restores surfacing.
    checkMock.mockResolvedValue(makeUpdate('2.1.0'));
    await upd.start();
