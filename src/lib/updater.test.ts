...
// Default module mocks: desktop, with the Tauri plugins stubbed so the dynamic
// imports inside updater.ts resolve without a native bridge.
...
// Dev-only fake backend (./updater.fake): when VITE_FAKE_UPDATE is set, the
// helpers delegate to a synthetic update + simulated install so the UI can be
// iterated without a server. updateChecker is unaware this exists.
...
describe('checkForUpdate', () => {
  ...
  it('maps version/notes/date from the plugin Update', async () => {
    ...
    // The opaque handle is the raw Update object, used later to install.
