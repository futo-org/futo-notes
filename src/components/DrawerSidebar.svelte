<script lang="ts">
  ...
  async function handleDropFolderOnFolder(folderPath: string, targetPath: string): Promise<void> {
...
<aside bind:this={drawerEl} class="notes-drawer" aria-hidden={!drawerOpen}>
  <div class="sidebar-header">
    <div class="sidebar-brand">
      <button class="brand-text" onclick={handleBrandClick}
  ...
  <div class="sidebar-view-toggle">
    <button
      ...
      onclick={() => {
        sidebarView = 'notes';
        localStorage.setItem('futo-notes:sidebarView', 'notes');
      }}
    >
      <svg
        width="16"
    ...
    <button
      class:active={sidebarView === 'tags'}
      aria-label="Tags view"
      onclick={() => {
        sidebarView = 'tags';
        localStorage.setItem('futo-notes:sidebarView', 'tags');
      }}
    >
      <svg
        width="16"
    ...
    <button
      class:active={sidebarView === 'images'}
      aria-label="Images view"
      onclick={() => {
        sidebarView = 'images';
        localStorage.setItem('futo-notes:sidebarView', 'images');
      }}
    >
      <svg
        width="16"
  ...
  {#if showCreateFolder}
    <CreateFolderModal
      ...
      validate={(v) =>
        validateNewFolderName(createFolderParent, v.trim(), collectSiblings(createFolderParent))}
