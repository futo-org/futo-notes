<script lang="ts">
  /**
   * Floating "update available" banner. Desktop-only; driven entirely by the
   * shared `updateChecker` state machine (auto-check on launch + hourly). Lives
   * in the global slot in App.svelte and self-gates via `bannerVisible`.
   *
   * One orange pill. Clicking it installs (or retries after an error); the
   * download → install → relaunch then runs to completion on its own, so there
   * is no second "restart" click. The pill is inert while that runs.
   */
  import { updateChecker as upd } from '$lib/updateChecker.svelte';

  function primary(): void {
    // 'available' or 'error' (retry). Disabled while busy / restarting, so this
    // can't fire mid-download. install() runs through to the auto-relaunch.
    void upd.install();
  }
</script>
...
<style>
  ...
  @media (prefers-reduced-motion: reduce) {
    ...
    .update-pill-bar-fill { transition: none; }
  }
</style>
