...
// A controllable stand-in for the shared updateChecker singleton. The component
// reads these fields and calls these methods; we mount fresh per phase, so no
// runtime reactivity is needed — set the fields, then mount and assert.
// vi.mock is hoisted above module init, so the fake must be hoisted too.
...
vi.mock('$lib/updateChecker.svelte', () => ({ updateChecker: upd }));
...
describe('UpdateBanner', () => {
  ...
  it('downloading: shows percent, a determinate bar, disables the pill', () => {
    ...
    // Icon shows only on the 'available' phase — gone once an install starts.
