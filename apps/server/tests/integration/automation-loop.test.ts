...
function createVault(rootDir: string): string {
  ...
  fs.writeFileSync(path.join(sourcePath, 'This week (3-10-2026 to 3-16).md'), 'We need to ship the project demo and prepare the automation review this week.\n', 'utf8');
...
describe('automation loop', () => {
  ...
  it('copies a vault, runs built-in automations, and writes diff artifacts', async () => {
    ...
    const sourcePath = createVault(tempRoot);
    ...
    const weekly = fs.readFileSync(path.join(result.workingVaultPath, 'This week (3-10-2026 to 3-16).md'), 'utf8');
