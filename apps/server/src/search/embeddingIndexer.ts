  ...
  return async (db: Database.Database, uuids: string[]) => {
    for (const uuid of uuids) {
      try {
        ...
        // Only index markdown files — skip images and other non-.md files
        if (!note.filename.endsWith('.md')) {
          ...
          log.debug(`search: skipping non-markdown ${uuid.slice(0, 8)} (${note.filename})`);
          continue;
        }
        ...
        log.warn(`search: failed to index ${uuid.slice(0, 8)}: ${message}`);
      }
    }
  };
}
