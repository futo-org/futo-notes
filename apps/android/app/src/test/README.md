...
```bash
just test-android-native   # UniFFI Kotlin bindings, then both flavors' unit tests
```

That is `:app:testDirectDebugUnitTest` **and** `:app:testPlayDebugUnitTest`:
there is no unflavored `testDebugUnitTest` since the `direct`/`play` split, and
`DistributionFlavorTest` asserts a per-flavor constant, so a single-flavor run
would only ever see half of it.

CI: `build:android-native` runs both on BOTH its paths —
