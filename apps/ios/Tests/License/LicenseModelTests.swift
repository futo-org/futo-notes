...
struct LicenseModelTests {
    ...
    // 2026-09-18). Desktop's copy is `license.svelte.ts`; Android's is
    // `LicenseModelTest`.

    @Test("a link that licenses the device leaves a celebration owed, exactly once")
    func activationOwesOneCelebration() {
        let (license, _) = model()
        license.showMessage = { _ in }

        #expect(!license.activationToCelebrate)
        ...
        #expect(!license.activationToCelebrate)
        // Idempotent: a second plate, or a re-render, owes nothing.
        license.markCelebrated()
        #expect(!license.activationToCelebrate)
    }

    /// Launching an app that was already licensed is not an activation. This is
    /// the whole reason the debt exists rather than a `status == .licensed`
    /// check.
    @Test("a license found on disk owes no celebration")
    func aStoredLicenseOwesNothing() async {
        let defaults = InMemoryLicenseDefaults()
        LicenseStorage(defaults: defaults).write(
            LicensePair(key: LicenseFixture.key, activation: LicenseFixture.activation))
        let license = LicenseModel(
    ...
    /// plain count this was the bug: the count stayed at 1 with nothing on
    /// screen to celebrate.
    @Test("removing a license leaves nothing owed")
    func removalOwesNothing() {
        let (license, _) = model()
        license.showMessage = { _ in }
    ...
    @Test("re-applying a link to a licensed device owes nothing more")
    func reapplyingALinkOwesNothingMore() {
        let (license, _) = model()
        license.showMessage = { _ in }
