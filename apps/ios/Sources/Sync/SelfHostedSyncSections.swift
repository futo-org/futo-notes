...
///
/// Extracted from `SyncView` unchanged so there is exactly one of it. A build
/// with hosted sync off renders these sections directly; a build with it on
/// discloses these same sections behind "Use my own server", rather than a
/// second copy that could drift from them (parent spec user story 34).
struct SelfHostedSyncSections: View {
    ...
    /// this view is built afresh each time the Sync sheet is presented, this
    /// initializer re-runs and re-seeds on every open. Only used on the
    /// not-connected reconnect path — the field is hidden once connected (the
    /// in-memory Rust session already holds the unwrapped vault key; `Sync Now`
    /// ignores it).
